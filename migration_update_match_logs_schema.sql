...
-- This migration adds missing fields and updates existing ones to match our requirements
...
-- 4. Fix data type issues - some fields are objects but should be text
-- Update user_career_path to be text if it's currently object
ALTER TABLE public.match_logs 
...
-- Update user_professional_expertise to be text if it's currently object
ALTER TABLE public.match_logs 
ALTER COLUMN user_professional_expertise TYPE TEXT USING 
  CASE 
    WHEN user_professional_expertise IS NULL THEN NULL
...
-- Update user_work_preference to be text if it's currently object
ALTER TABLE public.match_logs 
...
-- Update error_message to be text if it's currently object
ALTER TABLE public.match_logs 
...
SET user_career_path = 'Data & Analytics',
    user_professional_expertise = 'Data Analysis',
    user_work_preference = 'Remote'
WHERE user_email = 'test@example.com' AND user_career_path IS NULL;

UPDATE public.match_logs 
SET user_career_path = 'Finance & Investment',
    user_professional_expertise = 'Investment Banking',
