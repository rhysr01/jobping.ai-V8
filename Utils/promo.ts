 ...
 */
export function validatePromoCode(code: string | undefined | null, email: string): PromoValidation {
  ...
  if (provided !== configured) return { isValid: false, reason: 'invalid_code' };

  const allowRegex = process.env.PROMO_EMAIL_ALLOWLIST;
  if (allowRegex) {
    try {
      const re = new RegExp(allowRegex);
      if (!re.test(email)) return { isValid: false, reason: 'email_not_allowed' };
    } catch {
