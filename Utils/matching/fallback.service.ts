...
export function generateRobustFallbackMatches(jobs: any[], _user: UserPreferences): FallbackMatch[] {
...
export class FallbackMatchingService {
  ...
  generateRobustFallbackMatches(jobs: any[], user: UserPreferences, _maxMatches?: number): any[] {
    const scored = this.scoringService.scoreJobsForUser(jobs as any, user as any);
    const categorized = (this.scoringService as any).categorizeMatches(scored);
    const combined = [...(categorized.confident || []), ...(categorized.promising || [])];
  ...
  generateMatchesByCriteria(jobs: any[], user: UserPreferences, opts: { careerPath?: boolean; location?: boolean; maxResults?: number } = {}) {
    ...
    if (opts.careerPath && user.career_path) {
      const paths = Array.isArray(user.career_path) ? user.career_path.map(p => String(p).toLowerCase()) : [String(user.career_path).toLowerCase()];
      results = results.filter(j => paths.some(p => String(j.title || j.description || '').toLowerCase().includes(p)));
    }
    ...
    return base.slice(0, max).map((j, idx) => ({ job: j, match_score: Math.max(50, 85 - idx * 5), match_reason: 'Criteria match', match_quality: 'good' }));
  }

  generateEmergencyFallbackMatches(jobs: any[], _user: UserPreferences, _max?: number) {
    const now = Date.now();
    ...
    return recent.slice(0, MATCHING_CONFIG.fallback.maxEmergencyMatches).map((j, idx) => ({ job: j, match_score: 30 + Math.max(0, (recent.length - idx) * 5), match_reason: 'Recent opportunity', match_quality: 'fair', confidence_score: 0.5 }));
  }

  shouldUseFallback(matches: Array<{ confidence_score?: number }>, _user: UserPreferences): boolean {
    if (!matches || matches.length === 0) return true;
    const avg = matches.reduce((s, m) => s + (m.confidence_score ?? 0.7), 0) / matches.length;
