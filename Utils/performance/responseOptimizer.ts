...
export class ResponseOptimizer {
  ...
  createOptimizedResponse(
    ...
    if (enableCaching && this.shouldCache(data)) {
      const cached = this.getFromCache(etag, cacheConfig?.ttl || this.defaultTTL);
      if (cached) {
  ...
  createErrorResponse(
    ...
    return this.createOptimizedResponse(errorData, {
      enableCaching: false,
      enableCompression: true
    }, status);
  }
  ...
  private addOptimizationHeaders(
    ...
    const { enableCompression, enableCaching, enableETag, maxAge, etag } = options;
  ...
  private addCacheHeaders(response: NextResponse, maxAge: number, etag: string, fromCache: boolean): void {
...
export function createOptimizedResponse(data: any, options?: ResponseOptimizationOptions) {
  return responseOptimizer.createOptimizedResponse(data, options);
}

export function createErrorResponse(error: string | Error, status?: number, details?: any) {
  return responseOptimizer.createErrorResponse(error, status, details);
}

export function createSuccessResponse(data: any, message?: string, options?: ResponseOptimizationOptions) {
...
export function createPaginatedResponse(
  ...
  return responseOptimizer.createPaginatedResponse(data, page, limit, total, options);
}
