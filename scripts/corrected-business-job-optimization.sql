...
UPDATE jobs 
SET 
  categories = ARRAY['internship', 'business-graduate', 'early-career'],
  is_internship = true
WHERE created_at > NOW() - INTERVAL '2 hours'
  AND (
...
UPDATE jobs 
SET 
  categories = ARRAY['graduate-program', 'business-graduate', 'early-career'],
  is_graduate = true
WHERE created_at > NOW() - INTERVAL '2 hours'
...
WHERE created_at > NOW() - INTERVAL '2 hours'
  AND LOWER(title) ~ '\b(graduate|trainee|associate|junior|entry\s*level)\b'
  AND NOT LOWER(title) ~ '\b(intern|internship|stage|prácticas|praktikum|tirocinio|stagista|stagiaire|becario|becaria)\b';

-- Set is_internship flag for internship roles
UPDATE jobs 
SET is_internship = true
WHERE created_at > NOW() - INTERVAL '2 hours'
  AND LOWER(title) ~ '\b(intern|internship|stage|prácticas|praktikum|tirocinio|stagista|stagiaire|becario|becaria|afstudeerstage|pflichtpraktikum)\b';
...
-- Set proper status for all business jobs
UPDATE jobs 
SET status = 'active'
WHERE created_at > NOW() - INTERVAL '2 hours'
