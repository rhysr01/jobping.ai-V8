...
describe('fallback.service', () => {
  ...
  describe('FallbackMatchingService', () => {
    ...
    it('should generate emergency matches', () => {
      const service = new FallbackMatchingService();
      const matches = service.generateEmergencyFallbackMatches(mockJobs, mockUser);
      expect(matches).toBeDefined();
      expect(Array.isArray(matches)).toBe(true);
    });
    ...
    it('should filter recent jobs for emergency matches', () => {
      ...
      const matches = service.generateEmergencyFallbackMatches([recentJob, oldJob], mockUser);
    ...
    it('should use fallback for low confidence matches', () => {
      ...
      expect(service.shouldUseFallback(lowConfidenceMatches, mockUser)).toBe(true);
    });
    ...
    it('should not use fallback for high confidence matches', () => {
      ...
      expect(service.shouldUseFallback(highConfidenceMatches, mockUser)).toBe(false);
    });
