...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const dbCheck = checkDatabase().catch(() => ({ status: 'degraded', message: 'Database check timeout' }));
    ...
    const dbStatus = await Promise.race([
      dbCheck,
      new Promise(resolve => setTimeout(() => resolve({ status: 'healthy', message: 'Database check in progress' }), 50))
    ]);
