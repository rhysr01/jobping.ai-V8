...
async function fillSignupForm(page: any, user: any, fastMode = false) {
  ...
  await page.waitForSelector('button:has-text("Continue to Preferences"):not([disabled])', { timeout: 5000 });
  ...
  // Step 3: Career path
  await page.locator(`button:has-text("${getCareerPathLabel(user.careerPath)}")`).click();
...
function getCareerPathLabel(careerPath: string): string {
  ...
  return careerPaths[careerPath] || 'Not Sure Yet / General';
}
