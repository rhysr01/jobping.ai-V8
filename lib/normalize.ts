...
export type JobTrack = 'consulting'|'finance'|'strategy'|'operations'|'marketing'|'product'|'data'|'sustainability'|'other';

export function inferTrack(text: string): JobTrack {
  const t = text.toLowerCase();
  if (/(consultant|consulting|advisory|strategy)/.test(t)) return "consulting";
  if (/(investment|bank|finance|equity|audit|account)/.test(t)) return "finance";
...
export function scoreJob(
  ...
  let early = 0;
  if (/(graduate programme|graduate program|graduate|entry level|intern|trainee|junior|analyst|rotation|leadership)/.test(s)) early += 20;
...
export function normalize(job: any): JobPingJob {
  ...
  const postedAt = job.posted_at ?? job.publication_date ?? new Date().toISOString();
  ...
  const hash = crypto.createHash("sha256")
    .update(`${companyDomain}|${job.external_id ?? url}|${title}|${locationName}|${postedAt}`)
    .digest("hex");

  return {
    id: hash,
    title, company, companyDomain,
    locationName, locationId,
    postedAt, seniority, source, url,
