...
export async function scrapeWellfound(runId: string): Promise<{ raw: number; eligible: number; careerTagged: number; locationTagged: number; inserted: number; updated: number; errors: string[]; samples: string[] }> {
  ...
  try {
    ...
    if (allJobs.length > 0) {
      try {
        ...
        console.error(`❌ Wellfound database error:`, errorMsg);
        telemetry.recordError(errorMsg);
      }
    }
    
    // Log standardized funnel metrics
    logFunnelMetrics('wellfound', telemetry.getTelemetry());
    
    return telemetry.getTelemetry();
    
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : 'Unknown error';
    console.error('❌ Wellfound scraping failed:', errorMsg);
    telemetry.recordError(errorMsg);
