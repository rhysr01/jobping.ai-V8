...
export async function scrapeIAgora(runId: string): Promise<{ raw: number; eligible: number; careerTagged: number; locationTagged: number; inserted: number; updated: number; errors: string[]; samples: string[] }> {
  ...
  try {
    ...
    // Scrape jobs for each EU country
    for (const country of EU_COUNTRIES) {
      console.log(`📍 Scraping iAgora for ${country.name}...`);
      
      try {
        const countryJobs = await circuitBreaker.execute(async () => {
          return await scrapeCountryJobs(country, runId, userAgent);
        });
        
        allJobs.push(...countryJobs);
        console.log(`✅ ${country.name}: ${countryJobs.length} jobs found`);
        
        // Rate limiting between countries
        await sleep(2000 + Math.random() * 3000);
        
      } catch (error) {
        console.error(`❌ Failed to scrape ${country.name}:`, error);
        continue;
      }
    }
...
async function scrapeCountryJobs(country: { code: string; name: string }, runId: string, userAgent: string): Promise<Job[]> {
  const jobs: Job[] = [];
  const baseUrl = 'https://www.iagora.com/work/jobs';
  ...
  try {
    ...
    try {
      ...
      const searchUrl = `${baseUrl}?country=${country.code}&type=graduate`;
      ...
      for (const element of jobElements.slice(0, 20)) { // Limit to 20 jobs per country
        try {
          const job = await processJobElement(page, element, country, runId, userAgent);
          if (job) {
            jobs.push(job);
          }
        } catch (error) {
    ...
    // Fallback to axios
    try {
      const response = await axios.get(`${baseUrl}?country=${country.code}`, {
      ...
      jobElements.each((index, element) => {
        if (index >= 20) return; // Limit to 20 jobs per country
        
        try {
          const job = processJobElementCheerio($, $(element), country, runId, userAgent);
          if (job) {
            jobs.push(job);
          }
        } catch (error) {
      ...
      console.error(`Axios fallback failed for ${country.name}:`, axiosError);
    }
  }
...
async function processJobElement(page: any, element: any, country: { code: string; name: string }, runId: string, userAgent: string): Promise<Job | null> {
  try {
    ...
    const location = await element.$eval('.job-location, .location, .city', (el: any) => el.textContent?.trim() || country.name);
...
function processJobElementCheerio($: cheerio.CheerioAPI, $el: cheerio.Cheerio<any>, country: { code: string; name: string }, runId: string, userAgent: string): Job | null {
  try {
    ...
    const location = $el.find('.job-location, .location, .city').first().text().trim() || country.name;
    ...
    const description = `International position at ${company} in ${location}. ${title}`;
    ...
    const job: Job = {
      ...
      company: company,
      location: location,
      job_url: jobUrl.startsWith('http') ? jobUrl : `https://www.iagora.com${jobUrl}`,
      description: description,
      experience_required: analysis.experienceLevel,
      work_environment: analysis.workEnv,
      language_requirements: analysis.languages,
