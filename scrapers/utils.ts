...
export function parseLocation(location: string): { city: string; country: string; isRemote: boolean; isEU: boolean } {
...
export function validateJob(job: IngestJob): { valid: boolean; errors: string[] } {
  ...
  if (!job.location || job.location.trim().length === 0) errors.push('Location is required');
  if (!job.description || job.description.trim().length === 0) errors.push('Description is required');
  if (!job.url || job.url.trim().length === 0) errors.push('URL is required');
  if (!job.source || job.source.trim().length === 0) errors.push('Source is required');
  return { valid: errors.length === 0, errors };
}

// Convert IngestJob to DB format
...
export function logJobProcessing(job: IngestJob, action: string, details?: any): void {
  const { isEU } = parseLocation(job.location);
  const isEarlyCareer = classifyEarlyCareer(job);
